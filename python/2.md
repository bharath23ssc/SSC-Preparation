...
```mermaid
flowchart TD
    A[🏰 MUGHAL EMPIRE UNDER SHAH JAHAN] --> B[💸 ECONOMIC CRISIS\nOverspending on monuments\nTaj Mahal, Red Fort, Shahjahanabad]
    B --> C[👑 AURANGZEB INHERITS BANKRUPT EMPIRE\n1658–1707]
    C --> D[📈 INCREASES TAXATION\nTo fix economic crisis]
    C --> E[🕌 RELIGIOUS INTOLERANCE\nRe-imposes Jizya\nAlienates Rajputs\nDestroys temples]
    D --> F[🌧️ EL NIÑO YEARS\n8–9 consecutive failures of monsoon\nCrop yields collapse]
    F --> G[🌾 AGRARIAN CRISIS\nPeasants revolt\nAbandon farming\nRefuse to pay taxes]
    G --> H[💰 REVENUE COLLAPSES FURTHER]
    B --> I[🏛️ JAGIRDARI CRISIS\nAurangzeb expands Khalisa land\nReduces Paibaqi — fewer jagirs\nMore jagirdars than jagirs available]
    I --> J[⚔️ ADMINISTRATIVE BREAKDOWN\nOfficers compete for jagirs\nCorruption, over-extraction\nGovernance collapses]
    E --> K[🤝 RAJPUT ALIENATION\nBiggest military pillar lost]
    H --> L[🗡️ NORTHWESTERN INVASIONS]
    J --> L
    K --> L
    L --> M[🇮🇷 NADIR SHAH 1739\nBattle of Karnal\nLoot of Delhi treasury]
    L --> N[🇦🇫 AHMAD SHAH ABDALI\n8 invasions 1748–1767]
    M --> O[👑 WEAK SUCCESSORS\nBA-JA-F-RA-MU-A-SHA-AK-BA-2]
    N --> O
    O --> P[🏴‍☠️ POLITICAL VACUUM IN INDIA]
    P --> Q[⚓ EUROPEANS ENTER INDIA]
...
```mermaid
flowchart LR
    R[🌍 WHY EUROPEANS CAME TO INDIA] --> S[🌶️ Pepper — Black Gold\nBest meat preservative]
    R --> T[🧵 Cotton & Silk\nSuperior quality textiles]
    S --> U[🛤️ LAND ROUTE BLOCKED\nOttoman Empire c.1400–1500]
    T --> U
    U --> V[⛵ NEED FOR SEA ROUTE\nLand route = costly / dangerous]
    V --> W[🌍 PROBLEM: AFRICA IN THE WAY\n90 years of failed attempts\n1400–1490]
    W --> X[🏁 BARTHOLOMEU DIAS 1488\nReaches Cape of Good Hope\nFunded by Henry the Navigator]
    X --> Y[⚓ VASCO DA GAMA 1498\nHires Gujarati pilot Abdul Majid\nReaches Calicut, India]
    Y --> Z[🚪 SEA ROUTE TO INDIA OPENED\nFloodgates for European colonisation]

    style R fill:#1a5276,color:#fff
    style Z fill:#1e8449,color:#fff
```
...
mindmap
  root((MUGHAL DECLINE\nCauses))
    Economic Crisis
      Shah Jahan's overspending
      Bankrupt treasury inherited by Aurangzeb
    Agrarian Crisis
      El Nino years — 8 to 9
      Monsoon failures
      Peasant revolts
